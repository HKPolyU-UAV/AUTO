...
static visualization_msgs::Marker vis_traj;
...
void position_callback(const geometry_msgs::PoseStamped::ConstPtr& pose)
{
    ...
    vis_traj.header.frame_id = "/map";
    vis_traj.header.stamp = ros::Time::now();
    vis_traj.ns = "GT_points";
    vis_traj.id = 0;
    vis_traj.action = visualization_msgs::Marker::ADD;
    vis_traj.pose.orientation.w = 1.0;
    vis_traj.type = visualization_msgs::Marker::SPHERE_LIST;
    vis_traj.scale.x = vis_traj.scale.y = vis_traj.scale.z = 0.1;
    ...
    ofstream save("tracklog.txt",ios::app);
    save<<uavinfo.x<<endl;
    save<<uavinfo.y<<endl;
    save<<uavinfo.z<<endl;
    save.close();

    vis_traj.points.push_back(uav);
}
...
static visualization_msgs::Marker vis_pooh;

void obj_vicon_cb(const geometry_msgs::PoseStamped::ConstPtr& pose)
{
    static geometry_msgs::Point pooh_in_vicon;
    pooh_in_vicon.x = pose->pose.position.x;
    pooh_in_vicon.y = pose->pose.position.y;
    pooh_in_vicon.z = pose->pose.position.z;

    vis_pooh.header.frame_id = "/map";
    vis_pooh.header.stamp = ros::Time::now();
    vis_pooh.ns = "GT_points";
    vis_pooh.id = 0;
    vis_pooh.action = visualization_msgs::Marker::ADD;
    vis_pooh.pose.orientation.w = 1.0;
    vis_pooh.type = visualization_msgs::Marker::SPHERE_LIST;
    vis_pooh.scale.x = vis_pooh.scale.y = vis_pooh.scale.z = 0.1;
    ...
    vis_pooh.points.push_back(pooh_in_vicon);

    ofstream save("pooh_for_track.txt",ios::app);
    save<<pooh_in_vicon.x<<endl;
    save<<pooh_in_vicon.y<<endl;
    save<<pooh_in_vicon.z<<endl;
    save.close();
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub_pooh = nh.subscribe<geometry_msgs::PoseStamped>
                               ("/vrpn_client_node", 10, obj_vicon_cb);
